function Get-ComprehensiveM365RoleAudit_old  {
    param(
        ...
        # Certificate-based Authentication Parameters (Recommended)
        [string]$TenantId,
        [string]$ClientId,
    ...
    try {
        ...
        if (-not $Organization -and $TenantId) {
            $Organization = $TenantId + ".onmicrosoft.com"
            Write-Host "Organization derived from TenantId: $Organization" -ForegroundColor Yellow
        }
        elseif (-not $Organization) {
            Write-Warning "Organization parameter is required for Exchange and Compliance audits"
            Write-Host "Either provide -Organization parameter or -TenantId" -ForegroundColor Yellow
        }
        ...
        elseif ($script:AppConfig.UseAppAuth -and $script:AppConfig.AuthType -eq "Certificate") {
        ...
        elseif ($script:AppConfig.UseAppAuth -and $script:AppConfig.AuthType -eq "ClientSecret") {
            Write-Warning "Client secret authentication is no longer supported"
            Write-Host "Please migrate to certificate-based authentication:" -ForegroundColor Yellow
            Write-Host "1. Run: New-M365AuditCertificate" -ForegroundColor White
            Write-Host "2. Upload certificate to Azure AD app registration" -ForegroundColor White
            Write-Host "3. Run: Set-M365AuditCertCredentials" -ForegroundColor White
            throw "Certificate-based authentication required"
        }
        else {
            Write-Warning "Certificate authentication not configured"
            Write-Host "Please configure certificate authentication:" -ForegroundColor Yellow
            ...
            Write-Host "• Or: Get-M365AuditRequiredPermissions for complete setup guide" -ForegroundColor White
            throw "Certificate-based authentication required"
        }
        ...
        if ($SharePointTenantUrl -like "*onmicrosoft.com*" -and -not ($SharePointTenantUrl -like "*sharepoint.com*")) {
            Write-Warning "SharePoint URL should be in format: https://yourtenant-admin.sharepoint.com"
            Write-Warning "Not: $SharePointTenantUrl"
            $tenantName = ($SharePointTenantUrl -split "//")[1] -replace "-admin\.onmicrosoft\.com", ""
            $SharePointTenantUrl = "https://$tenantName-admin.sharepoint.com"
            Write-Host "Corrected SharePoint URL: $SharePointTenantUrl" -ForegroundColor Yellow
        }
        ...
        if (-not (Get-Module -ListAvailable -Name "Microsoft.Graph.Authentication")) {
            $installationNeeded += "Microsoft.Graph.Authentication"
        }
        if (-not (Get-Module -ListAvailable -Name "Microsoft.Graph.Users")) {
            $installationNeeded += "Microsoft.Graph.Users"
        }
        if (-not (Get-Module -ListAvailable -Name "Microsoft.Graph.Identity.DirectoryManagement")) {
            $installationNeeded += "Microsoft.Graph.Identity.DirectoryManagement"
        }
        
        # Check PnP PowerShell for SharePoint
        if (($IncludeSharePoint -or $IncludeAll) -and -not (Get-Module -ListAvailable -Name "PnP.PowerShell")) {
            $installationNeeded += "PnP.PowerShell"
        }
        
        # Check Exchange Online module
        if (($IncludeExchange -or $IncludePurview -or $IncludeAll) -and -not (Get-Module -ListAvailable -Name "ExchangeOnlineManagement")) {
            $installationNeeded += "ExchangeOnlineManagement"
        }
        
        # Check Teams module
        if (($IncludeTeams -or $IncludeAll) -and -not (Get-Module -ListAvailable -Name "MicrosoftTeams")) {
            $installationNeeded += "MicrosoftTeams"
        }
        
        # Install missing modules
        if ($installationNeeded.Count -gt 0) {
            Write-Host "Installing required modules: $($installationNeeded -join ', ')" -ForegroundColor Yellow
            foreach ($module in $installationNeeded) {
                try {
        ...
        # Validate certificate if using certificate authentication
        if ($script:AppConfig.UseAppAuth -and $script:AppConfig.AuthType -eq "Certificate") {
            ...
            if ($cert) {
                ...
                if ($isValid) {
                    Write-Host "✓ Certificate is valid (expires: $($cert.NotAfter))" -ForegroundColor Green
                }
                else {
                    Write-Warning "Certificate is expired or not yet valid"
                    Write-Host "Certificate validity: $($cert.NotBefore) to $($cert.NotAfter)" -ForegroundColor Red
                    throw "Certificate validation failed"
                }
            }
            else {
                throw "Certificate with thumbprint $($script:AppConfig.CertificateThumbprint) not found"
            }
        }
